...
├── desktop/                    # Electron shell, preload bridge, active and legacy renderers, desktop tests
├── docs/                       # Release docs, reports, plans, and UI/design artifacts
├── scripts/                    # Top-level setup helpers for local installs
├── .planning/codebase/         # Generated GSD codebase mapping documents
├── dist/                       # Packaged desktop build outputs
└── stitch_exports/             # User-generated export artifacts outside the backend job store
```
...
- Contains: Electron main/preload files, packaging config, build resources, tests, legacy renderer, and `renderer-v2`.
...
**`desktop/renderer/`:**
- Purpose: Legacy renderer kept as a fallback UI.
- Contains: a large `app.js`, `index.html`, CSS, and helper modules under `desktop/renderer/modules/`.
- Key files: `desktop/renderer/app.js`, `desktop/renderer/index.html`, `desktop/renderer/modules/job-api.js`
...
- Contains: release builder wrapper, packaged-release validator, renderer-v2 structural checks, locale/profile checks.
- Key files: `desktop/scripts/run-builder.js`, `desktop/scripts/validate-packaged-release.js`, `desktop/scripts/check-renderer-v2.js`
...
- Contains: plans, specs, and design-system notes for major UI and workflow efforts.
- Key files: `docs/superpowers/plans/2026-04-17-youtube-prepare-progress.md`, `docs/superpowers/uiux/design-system/drum-sheet-capture/MASTER.md`
...
- `desktop/renderer/app.js`: Legacy renderer bootstrap.
...
- Legacy renderer modules also use camelCase filenames such as `desktop/renderer/modules/job-api.js` and `desktop/renderer/modules/video-range-picker.js`.
...
- Legacy fallback-only UI changes: keep them in `desktop/renderer/` and `desktop/renderer/modules/`; do not mix new fallback logic into `renderer-v2`.
