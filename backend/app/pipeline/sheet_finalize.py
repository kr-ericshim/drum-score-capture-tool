...
def finalize_sheet_sequence(
    ...
    *,
    ...
    if not pages:
        pages = [merged]
    page_frames = _frame_pages_as_printed_set(pages, page_ratio=page_ratio)
    return page_frames, merged, len(prepared_frames)


def _frame_pages_as_printed_set(images: List[np.ndarray], *, page_ratio: float) -> List[np.ndarray]:
    valid_images = [image for image in images if image is not None and image.size > 0]
    if not valid_images:
        return []

    max_w = max(int(image.shape[1]) for image in valid_images)
    max_h = max(int(image.shape[0]) for image in valid_images)
    if max_w <= 0 or max_h <= 0:
        return valid_images

    margin_x = max(10, int(max_w * 0.018))
    margin_top = max(14, int(max_h * 0.024))
    margin_bottom = max(20, int(max_h * 0.036))
    canvas_w = max_w + (margin_x * 2)
    ...
    common_scale = min(1.0, inner_w / float(max_w), inner_h / float(max_h))

    return [
        _frame_as_printed_page(
            image,
            canvas_w=canvas_w,
            canvas_h=canvas_h,
            margin_x=margin_x,
...
def _split_long_page(
    ...
    *,
    ...
    min_single_page_scale = 0.96 if mode == "performance" else 0.97
    ...
    # Band-based grouping is only safe when page ranges stay effectively contiguous.
    # If it introduces internal dropped rows, fall back to whitespace slicing so
    # sparse notation between systems is preserved.
    max_internal_gap = max(6, int(target_h * 0.004))
    if _has_internal_page_gaps(pages, max_gap=max_internal_gap):
        return _slice_by_whitespace(
            image,
            row_density=row_density,
            target_h=target_h,
    ...
    if normalized_pages:
        return _balance_short_edge_pages(
            normalized_pages,
            target_h=target_h,
            page_fill_mode=mode,
            page_ratio=page_ratio,
...
def _has_internal_page_gaps(ranges: List[Tuple[int, int]], *, max_gap: int) -> bool:
    if len(ranges) < 2:
        return False
...
def _balance_short_edge_pages(
    ...
    *,
    ...
    pages = _merge_short_leading_page(
        pages,
        target_h=target_h,
        page_fill_mode=page_fill_mode,
        page_ratio=page_ratio,
...
def _merge_short_leading_page(
    ...
    *,
    ...
    result = list(pages)
    if mode == "performance":
        max_head = max(120, int(target_h * 0.34))
        max_next = int(target_h * 1.10)
        min_merged_scale = 0.91
    else:
        max_head = max(120, int(target_h * 0.24))
        max_next = int(target_h * 1.02)
        min_merged_scale = 0.94

    first = result[0]
    second = result[1]
    first_h = int(first.shape[0])
    second_h = int(second.shape[0])
    if first_h > max_head or second_h > max_next:
        return result

    if page_ratio is not None:
        merged_scale = _estimate_single_page_frame_scale(
            height=first_h + second_h,
            width=int(max(first.shape[1], second.shape[1])),
            page_ratio=page_ratio,
        )
        if merged_scale < min_merged_scale:
            return result

    result[1] = np.vstack((_pad_to_common_width(first, second), _pad_to_common_width(second, first)))
    result.pop(0)
    return result


def _pad_to_common_width(image, other) -> np.ndarray:
    width = max(int(image.shape[1]), int(other.shape[1]))
    if int(image.shape[1]) >= width:
        return image
    delta = width - int(image.shape[1])
    left = delta // 2
    right = delta - left
...
def _refine_cut_boundary(
    *,
    ...
    total_h = int(row_density.shape[0])
    cut = int(np.clip(cut, start + 1, total_h))
    if cut - start < min_h:
        return cut

    def boundary_score(index: int) -> float:
        lo = max(0, index - 12)
        hi = min(total_h, index + 12)
    ...
    for candidate in range(candidate_lo, candidate_hi + 1):
        score = boundary_score(candidate) + (abs(candidate - hard_end) * 0.00018)
        if score < best_score:
            best_cut = candidate
            best_score = score

    improved = best_score < (current_score * 0.92)
    return int(best_cut if improved else cut)
...
def _estimate_single_page_frame_scale(*, height: int, width: int, page_ratio: float) -> float:
    ...
    margin_x = max(10, int(width * 0.018))
    margin_top = max(14, int(height * 0.024))
    margin_bottom = max(20, int(height * 0.036))
...
def _frame_as_printed_page(
    ...
    *,
    ...
    if margin_top is None:
        margin_top = max(14, int(h * 0.024))
    if margin_bottom is None:
        margin_bottom = max(20, int(h * 0.036))
    ...
    scale = float(np.clip(common_scale, 0.05, 1.0))
    if scale < 0.999:
    ...
    y0 = margin_top
    usable_w = max(1, canvas_w - (margin_x * 2))
    x0 = margin_x + max(0, (usable_w - w) // 2)
    if y0 + h > canvas_h - margin_bottom:
        y0 = max(margin_top, canvas_h - margin_bottom - h)
