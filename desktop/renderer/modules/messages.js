...
export function friendlyApiError(detail) {
  ...
  if (typeof detail === "string" && detail.startsWith("preview frame extraction failed:")) {
    const reason = detail.replace("preview frame extraction failed:", "").trim();
    if (reason) {
      return getLocale() === "ko"
        ? `영역 지정 화면 생성 실패: ${reason}. 시작 시간을 5~10초 정도로 옮겨 다시 시도합니다.`
        : `Could not generate the preview frame: ${reason}. Move the start time to around 5-10 seconds and try again.`;
    }
    return getLocale() === "ko"
  ...
  if (detail === "capture crop is unavailable after review export") {
    return getLocale() === "ko"
      ? "검토 반영으로 다시 만든 결과 페이지는 현재 다시 자르기를 지원하지 않습니다. 원본 캡처 단계에서 조정한 뒤 다시 생성합니다."
      : "Recrop is not available for pages that were regenerated by review export. Adjust the original capture step and regenerate instead.";
  }
  if (detail === "review export is already applied") {
    return getLocale() === "ko"
      ? "검토 반영은 이미 한 번 적용되었습니다. 현재 결과는 PDF 또는 결과 폴더 열기로 확인하고, 다시 고르려면 원본 작업을 다시 실행합니다."
      : "Review export has already been applied. Open the PDF or output folder for the current result, or rerun the original job to review again.";
  }
